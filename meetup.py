...
class MeetupImages(object):
    ...
    def downloadphoto(self, members):
        name = members['name']
        with open(f'./WorkingFolder/MeetupImages/{name}.jpeg', 'wb') as imagefile:
            imagefile.write(requests.request("GET", url=members['photo']['photo_link']).content)

    def getmembers(self, group_url):
        key = self.api_key
        requesturl = f'https://api.meetup.com/2/members?group_urlname={group_url}&key={key}'
        headers = {
